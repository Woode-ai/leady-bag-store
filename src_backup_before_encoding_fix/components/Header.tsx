  ...
  useEffect(() => {
    ...
    async function loadNotifications() {
      try {
        ...
        // Ø¥Ø°Ø§ Ù„Ù… ÙŠÙƒÙ† Ù…Ø³Ø§Ø± Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª Ø¬Ø§Ù‡Ø²Ø§Ù‹ Ø¨Ø¹Ø¯
      }
    }
    loadNotifications();
  }, [user]);
    ...
    <header className="border-b border-gray-100 bg-white/95 backdrop-blur-md sticky top-0 z-50 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <nav className="hidden lg:flex items-center gap-6 ms-4">
          ...
          >
            <Sparkles size={14} />
            <span>ØªÙ†Ø³ÙŠÙ‚ Ø§Ù„Ø¥Ø·Ù„Ø§Ù„Ø© (Looks)</span>
          </Link>
        </nav>
        ...
        <div className="flex items-center gap-3 ms-auto">
          ...
          {/* Ø¬Ø±Ø³ Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª Ø§Ù„Ø°ÙƒÙŠ */}
          {user && (
                ...
                <div className="absolute end-0 mt-3 w-80 sm:w-96 bg-white border border-gray-100 rounded-3xl shadow-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="flex items-center justify-between pb-3 border-b border-gray-100 mb-2">
                    <h3 className="font-bold text-secondary text-sm flex items-center gap-1.5">
                      <Bell size={16} className="text-primary" /> Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª ÙˆØ§Ù„ØªÙ†Ø¨ÙŠÙ‡Ø§Øª
                    </h3>
                    {unreadCount > 0 && (
                      <button
                      ...
                      >
                        <Check size={12} /> Ù‚Ø±Ø§Ø¡Ø© Ø§Ù„ÙƒÙ„
                      </button>
                    )}
                  </div>
                  ...
                  <div className="max-h-72 overflow-y-auto space-y-2">
                    {notifications.length === 0 ? (
                      <p className="text-center text-gray-400 text-xs py-8">Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¥Ø´Ø¹Ø§Ø±Ø§Øª Ø¬Ø¯ÙŠØ¯Ø© Ø­Ø§Ù„ÙŠØ§Ù‹</p>
                    ) : (
                      notifications.map((n) => (
                        <div
                          key={n._id}
