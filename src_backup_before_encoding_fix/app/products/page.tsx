...
function ProductsContent() {
  ...
  // Ø§Ù„Ø¨Ø­Ø« Ø¨Ø§Ù„Ø§Ø³Ù… ÙÙ‚Ø· ÙˆØ§Ù„ÙØ±Ø²
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      {/* Ø±Ø£Ø³ Ø§Ù„ØµÙØ­Ø© Ø§Ù„Ø¹ØµØ±ÙŠ */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-gradient-to-r from-primary/10 via-rose-50 to-primary/5 p-6 rounded-2xl border border-primary/15 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            {lang === "ar"
              ? "Ø§Ø³ØªÙƒØ´ÙÙŠ Ø£Ø­Ø¯Ø« ØªØ´ÙƒÙŠÙ„Ø§Øª Ø§Ù„Ø£Ø²ÙŠØ§Ø¡ ÙˆØ§Ù„Ø­Ù‚Ø§Ø¦Ø¨ Ø§Ù„Ù†Ø³Ø§Ø¦ÙŠØ© Ø§Ù„ÙØ±ÙŠØ¯Ø©"
              : "Discover the latest exclusive women's fashion and handbags collection"}
          </p>
        </div>

        {/* Ø§Ù„Ø¨Ø­Ø« Ø¹Ù† Ø·Ø±ÙŠÙ‚ Ø§Ø³Ù… Ø§Ù„Ù…Ù†ØªØ¬ ÙÙ‚Ø· */}
        <form onSubmit={handleSearchSubmit} className="relative flex-1 max-w-md">
          <input
            type="text"
            placeholder={lang === "ar" ? "Ø§Ø¨Ø­Ø«ÙŠ Ø¨Ø§Ø³Ù… Ø§Ù„Ù…Ù†ØªØ¬..." : "Search by product name..."}
            value={searchQuery}
      ...
      {/* Ø´Ø±ÙŠØ· Ø§Ù„ØªØ­ÙƒÙ… Ø¨Ø§Ù„ØªØ±ØªÙŠØ¨ */}
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
        <div className="text-sm text-gray-500">
          {(pagination as any).total ? `${(pagination as any).total} ${lang === "ar" ? "Ù…Ù†ØªØ¬ Ù…ØªØ§Ø­" : "products found"}` : ""}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-gray-50/50 rounded-2xl border border-gray-100">
            ...
            <p className="text-gray-400 text-sm">
              {lang === "ar" ? "Ø¬Ø±Ø¨ÙŠ Ø§Ù„Ø¨Ø­Ø« Ø¨ÙƒÙ„Ù…Ø§Øª Ø£Ø®Ø±Ù‰ Ø£Ùˆ ØªØµÙØ­ ÙƒÙ„ Ø§Ù„Ù…Ù†ØªØ¬Ø§Øª" : "Try searching with different keywords"}
            </p>
          </div>
        ) : (
          <>
...
// Suspense Ù…Ø·Ù„ÙˆØ¨ Ù„Ø£Ù† useSearchParams ÙŠØ­ØªØ§Ø¬Ù‡Ø§ Next.js Ø¹Ù†Ø¯ Ø§Ù„ØªØµØ¯ÙŠØ± Ø§Ù„Ø«Ø§Ø¨Øª
