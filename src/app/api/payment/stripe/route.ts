...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { status: "error", message: "فشل إنشاء جلسة الدفع", ...(process.env.NODE_ENV !== "production" && { error: (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) }) },
      { status: 500 }
    );
  }
}
