...
export async function POST(req: NextRequest) {
  try {
    ...
    // نتأكد أن هذا الطلب يخص المستخدم الحالي فعلاً - وإلا يستطيع أي عميل مسجّل دخوله
    // بدء جلسة دفع لأي رقم طلب يخمّنه (حتى لو لم يكن طلبه هو)
    ...
    return NextResponse.json(
      { status: "error", message: "فشل إنشاء جلسة الدفع عبر PayMob", ...(process.env.NODE_ENV !== "production" && { error: (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) }) },
      { status: 500 }
    );
  }
}
