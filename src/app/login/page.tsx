...
export default function LoginPage() {
  ...
  async function handleSendResetCode(e: React.FormEvent) {
    ...
    try {
      ...
      setForgotMsg(data.message || "تم إرسال رمز استعادة كلمة المرور إلى بريدك");
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      alert(data.message || "تم تغيير كلمة المرور بنجاح! يمكنك تسجيل الدخول الآن.");
      ...
      {showForgotModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-gray-100">
            <h2 className="text-xl font-bold text-secondary mb-1">استعادة كلمة المرور</h2>
            <p className="text-xs text-gray-500 mb-4">
              {forgotStep === "send_code"
                ? "أدخلي بريدك الإلكتروني لإرسال رمز تحقق سري من 6 أرقام"
                : "أدخلي رمز التحقق المستلم وكلمة المرور الجديدة"}
            </p>
